...
TIME_LIMIT_INNER = 900
# TIME_LIMIT_INNER= 120
tolerance_outer = 1.0e-5
tolerance_inner = 1.0e-6
...
beta = 0.7
# noise_parameter = [1.0e-8, 1.0e-7, 1.0e-6, 1.0e-5, 1.0e-4, 1.0e-3, 1.0e-2, 1.0e-1]
noise_parameter = np.power(10, np.linspace(-8, -3, 11)).tolist()
...
for _ in range(num_sets):
    ...
    for i in range(num_repetitions):
        for j in range(len(noise_parameter)):
            ...
            max_time = 1.0
            list_X, list_first_Y, list_Y, list_t = fermi_pasta_ulam_time_correct(
            ...
            list_X_noisy = [x + noise for x, noise in zip(list_X, list_noise)]
            ...
            first_derivative_norm[j, i] = np.linalg.norm(X_noisy_cropped)
            delta_norm[j, i] = np.linalg.norm(Y_noisy_cropped)
            ...
            # Need to do some cropping so that the Psi and first derivatives match in shape.
            list_Psi = [x[:, width:-width] for x in list_Psi]
            list_t = [x - x[0] for x in t_cropped]

            from auxiliary_functions import compute_integral_formulation_matrix

            width = 5
            # Here we need to provide the list of first derivatives, since the it is a 2nd order ODE.
            ...
            from dynamics import add_constraints_symmetry_fermi_pasta_ulam_tsingou
            from feasible_regions import l1_ball_pyscipopt

            equality_constraints = add_constraints_symmetry_fermi_pasta_ulam_tsingou(
            ...
            equality_constraints_integral = (
                add_constraints_symmetry_fermi_pasta_ulam_tsingou(
